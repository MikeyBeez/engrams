#!/usr/bin/env python3
"""
Multi-Topic Engram Benchmark

Tests whether topic-matched engrams at 3x strength consistently improve
Q&A accuracy on facts that are definitely in training data.

Design:
- 5 topics with 20 questions each = 100 questions total
- Each topic has a knowledge document (engram source)
- Test conditions: baseline, matched-engram, mismatched-engram
- All facts are common knowledge (definitely in training)

The key hypothesis: matched engram >> mismatched engram >> baseline
"""
...
print("\n" + "=" * 100)
print("MULTI-TOPIC ENGRAM BENCHMARK")
print("=" * 100)
...
# (rotate through topics for fair comparison)
...
for topic_idx, (topic_name, topic_data) in enumerate(TOPICS.items()):
    ...
    for q_data in topic_data["questions"]:
        ...
        print(f"  Base={b} Match={m} Mismatch={x} | {question[:60]}...")
...
# Save results
results_file = f"/home/bee/engrams/results/multi_topic_benchmark_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
